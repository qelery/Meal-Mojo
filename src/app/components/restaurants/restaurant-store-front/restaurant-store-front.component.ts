...
import { MenuItem, Restaurant } from '@shared/model';
...
export class RestaurantStoreFrontComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe((params) => {
      ...
      this.orderService.getRestaurantData(this.restaurantId).subscribe(
        (response: any) => {
          ...
          const hours = response.operatingHoursList.find(
            (hour: any) => hour.dayOfWeek === this.currentDayOfWeek.toUpperCase()
          );
          this.openTime = hours.openTime
            ? this.convertToShortTime(hours.openTime)
            : 'Unknown';
          this.closeTime = hours.closeTime
            ? this.convertToShortTime(hours.closeTime)
            : 'Unknown';
        },
  ...
  convertToShortTime(time: string) {
    let timeParts = time.toString().match(/^([01]\d|2[0-3])(:)([0-5]\d)(:[0-5]\d)?$/) || [
      time,
    ];
